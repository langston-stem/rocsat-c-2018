...
// Global variables
// GC out put is connected to pin 18
   const unsigned int gc_pin = 18;

// Attach 5th interrupt of the Ardurino mega.
   const unsigned int gc_intnumber = 5;

// Declared volatile because two threads of execu
// Value is initially set to zero because there
   volatile unsigned int gc_counts = 0;

// gc_cnt to store the number o counts 
...
void setup() {
  ...
  // Sets up the interrupt to trigger for a rising
  // gc_pin (18) corresponds to the 5th interrupt 
  // gc_interrupt is the function we want to call
