...
namespace Bmp{
 ...
 boolean bmp085WaitForBits(int sizeOFIn=1){
   bmperror_curCount=0;
   while(Wire.available()<sizeOFIn&&bmperror_curCount<bmperror_minCount){
     bmperror_curCount++;
   }
   if(bmperror_curCount>=bmperror_minCount){
     bmp085AddError();
     return false;
   }
   return true;
 ...
 int bmp085ReadInt(unsigned char address)
 {
   ...
   int counter=0;
   if(!bmp085WaitForBits(2)){return 0;}
   msb = Wire.read();
   lsb = Wire.read();
 ...
 unsigned long bmp085ReadUP()
 {
   ...
   if(!bmp085WaitForBits(3)){return 0;}
